...
interface SceneDetailPanelProps {
  scene: Scene;
  onBack: () => void;
  onUpdateScene: (field: keyof Scene, value: any) => void;
...
export const SceneDetailPanel: React.FC<SceneDetailPanelProps> = ({
  ...
  useEffect(() => {
    loadHistoryAssets();
    prevStatusRef.current = scene.status;
  }, [scene.id]);

  /**
   * 监听场景状态变化，生成完成后刷新历史记录
   */
  useEffect(() => {
    ...
    prevStatusRef.current = scene.status;
  }, [scene.status, scene.imageUrl]);

  /**
   * 加载历史资源记录
   */
  const loadHistoryAssets = async (showLoading = true) => {
    const numericId = parseInt(scene.id);
    ...
    try {
      const assets = await assetService.getAssets(numericId, 'scene_image', 'image');
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${scene.name || 'scene'}.png`;
    ...
    <div className="w-full h-full bg-bg-card flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        <div className="p-4 space-y-4">
                          ...
                          try {
                            ...
                            if (projectId) {
                              await assetService.createAsset({
                                ...
                                type: 'image',
                                usage: 'scene',
                                relatedId: parseInt(scene.id) || undefined,
                                url: url,
                                source: 'upload'
                              });
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-text-secondary gap-3">
                  {scene.status === 'failed' ? (
                    ...
                    <>
                      ...
                      <span className="text-sm opacity-70">暂无场景图</span>
                    </>
                  )}
                  {!uploading && (
                    <Upload
          ...
          {/* Scene Name */}
          <div>
            <div className="text-text-secondary text-xs mb-2">场景名称</div>
            <Input
              value={scene.name}
          ...
          {/* Scene Description */}
          <div>
            <div className="text-text-secondary text-xs mb-2">场景描述</div>
            <TextArea
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              loading={(scene.status as string) === 'generating'}
              disabled={(scene.status as string) === 'generating' || uploading}
            ...
            >
              {(scene.status as string) === 'generating' ? '生成中...' : (scene.imageUrl ? '重新生成' : '生成场景图')}
            </Button>
          </div>
                  ...
                  >
                    <div
                      className={`aspect-video bg-bg-element rounded-lg border overflow-hidden cursor-pointer transition-all relative group/history ${
                        asset.url === scene.imageUrl
                          ? 'border-primary ring-2 ring-primary/20'
                          : 'border-border hover:border-primary/50'
                      }`}
                      onClick={() => {
                        if (asset.url) {
                          onUpdateScene('imageUrl', asset.url);
                          onUpdateScene('status', 'generated');
                              ...
                              items: [
                                {
                                  ...
                                  onClick: async (e) => {
                                    ...
                                    try {
                                      ...
                                      link.href = url;
                                      link.download = `scene-${scene.name}-${asset.id}.png`;
